...
const ControlHelp = (function() {
  ...
  const SETTING_HELP = {
    ...
    warningsOverlay: 'Draw NWS-style watches, mesoscale discussions, and warnings as horizontal boxes from the surface upward. Products re-evaluate about every 12 sim-minutes (a confirmed tornado detection can issue a warning immediately). Tornado warnings come only from those detections — not Labels tornado tags or CAPE/STP alone. Boxes only appear on storm-focused corridors — not domain-wide CAPE. Overlapping products prefer Tornado Warning, then Flash Flood, then Severe Thunderstorm, then watches, then MDs.',
