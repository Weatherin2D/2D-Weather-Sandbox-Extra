...
const ControlHelp = (function() {
  ...
  const TOOL_HELP = {
    ...
    TOOL_WALL_URBAN: {
      title: 'Urban',
      body: 'Dense city surfaces with building facades. Choose a look with Urban Type (downtown, mid-rise, historic, waterfront, and more). Heats quickly during the day and can enhance local convection and storms. Building detail stays visible at all zoom levels. Hold Ctrl to remove any urban look.',
      keys: '1',
    },
    TOOL_WALL_SUBURBAN: {
      title: 'Suburban',
      body: 'Lighter built-up areas with house silhouettes between city and countryside. Choose a look with Suburban Type (gabled homes, American tract, townhouses, ranch, and more). Moderate heat retention and evaporation except American Tract, which behaves like Urban. Hold Ctrl to remove any suburban look.',
      keys: '2',
    },
    TOOL_WALL_AMERICAN_SUBURBAN: {
      title: 'American Suburban',
      body: 'Cookie-cutter American suburban houses (now the American Tract suburban type). Behaves like Urban for heating, smog, and vegetation limits, with a lower house skyline. Hold Ctrl to remove.',
      keys: '',
    },
    TOOL_WALL_RUNWAY: {
      title: 'Runway',
  ...
  const SETTING_HELP = {
    ...
    sunIntensity: 'Strength of incoming solar radiation. 1.0 is Earth\'s solar constant (1361 W/m² at the top of the atmosphere).',
    ...
    soilMoistureLossMult: 'How quickly soil dries by evaporation. 1 is default; 0 stops soil drying.',
    limitSoilMoisture: 'When on, soil moisture cannot exceed Max Soil Moisture (mm). Off (default) means unlimited storage; evaporation is limited only by available moisture.',
    maxSoilMoistureMm: 'Optional soil moisture ceiling in mm. Only applied when Limit Soil Moisture is on.',
    climateMoistureDecayMult: 'How quickly long-term climate moisture fades without rain. 1 is default.',
    vegetationGrowthMult: 'Speed of grass and forest growth from climate moisture and sunlight. 1 is default.',
    vegetationDiebackMult: 'Speed of drought dieback. 1 is default; higher values kill vegetation faster in dry conditions.',
    fireBurnMult: 'How quickly fire consumes vegetation. 1 is default; fire still goes out when fuel is gone or rain/flood hits.',
    rainfallAmountMult: 'How much rain soaks into soil and becomes runoff. 1 is default; does not change droplet spawn rate.',
    ...
    cameraMode: 'Free Camera is manual pan/zoom. Cinematic Camera automatically pans toward the strongest updraft (highest vertical velocity) and frames the storm. Zoom still works; arrow-key and middle-mouse panning are disabled while cinematic is on.',
    ...
    sound: 'Enable environmental wind, rain, ambient, and thunder audio. Does not control EAS warning tones — use EAS Sound for those.',
    easSoundEnabled: 'Play EAS attention tones and spoken warning crawls even when Enable Sound is off. Turn this off to keep the visual crawl without EAS audio.',
    ...
    highResPerformanceMode: 'Optional aggressive throttling. Not turned on automatically for 12000×200-class grids. Use Advanced → Apply Performance Preset if the GPU is actually struggling.',
    showDebugOverlay: 'Minecraft-style F3 debug HUD — FPS, frame time, resolution, camera, quality, and sim stats. Toggle with F3.',
    applyPerformancePreset: 'One-click aggressive speed settings for high resolutions (reduced precip, skip CAPE, lightning Performance tier, auto scaling). Starts at a low iter count so the GPU does not reset, then auto-iter climbs. Curl/vorticity stay on for turbulent flow.',
    ...
    enableFlooding: 'Allow persistent/frequent rain to create standing floodwater on land. One-off heavy totals alone do not flood.',
    enableStormSurge: 'Allow strong onshore winds to push ocean water onto low coasts as storm surge.',
    floodRainThreshold: 'How often it must be raining at a cell (0–1 rain-occurrence frequency) before ponding starts. Higher = needs more persistent rain.',
    floodPondRate: 'How quickly standing floodwater builds once rain frequency exceeds the flood rain frequency threshold.',
    displayMeteogram: 'Show the time-height meteogram for the active weather station (or double-click a station). Includes surface wind/MSLP/precip and a convective strip (CAPE, CIN, SRH, precip). After Forecast Now, the meteogram is filled from forecast keyframes.',
    surfacePressure: 'Sea-level baseline (hPa) for diagnostic MSLP and hydrostatic CAPE surface pressure. Fluid solver pressure is unchanged.',
    ...
    useHydrostaticCapePressure: 'Use column-integrated hydrostatic P(z) for CAPE/skew-T instead of altitude-only ISA. Fluid PRESSURE solver is unchanged.',
    recordIntervalSimSec: 'While recording, save a full-state keyframe every N simulation seconds (not every iteration).',
    replayMaxKeyframes: 'Ring-buffer cap for live recording keyframes. Older frames drop when the limit is hit.',
    replayPlaybackSpeed: 'How fast Play advances between keyframes in the replay/forecast scrubber.',
    forecastLeadHours: 'How far ahead Forecast Now free-runs the model before opening the forecast viewer.',
    startReplayRecording: 'Start capturing full simulation state keyframes (grids, precip, light, charge).',
    stopReplayRecording: 'Stop recording, download a .wsreplay file, and enter scrubbing mode.',
    loadReplayFile: 'Load a .wsreplay or .wsforecast file and scrub with free camera.',
    exportReplaySession: 'Download the current replay/forecast session without stopping recording.',
    exitReplayMode: 'Leave replay/forecast view. Forecast exit restores the initial-condition snapshot.',
    runForecastNow: 'Snapshot the current state, free-run to the forecast lead time, then scrub valid-time products. Exit restores the IC.',
    ...
    tornadoDetectionOverlay: 'Tag a tornado only when a narrow near-ground inbound wind couplet (about 49+ mph ΔV) has cloud to the surface and shows up on two scans. Intensity is that ΔV with an EF estimate. Separate from the Labels overlay tornado tag, which is environmental and does not warn.',
    ...
    warningsOverlay: 'Draw NWS-style watches, mesoscale discussions, and warnings as horizontal boxes from the surface upward. Mesoscale discussions cover the broader favorable environment (CAPE and shear), not just precip cores. Click a box with the Flashlight (no paint tool) to read the discussion. F4 toggles this overlay (rebindable). Products re-evaluate about every 12 sim-minutes (a confirmed tornado detection can issue a warning immediately). Tornado warnings come only from those detections. Overlapping products prefer Tornado Warning, then Flash Flood, then Severe Thunderstorm, then watches, then MDs.',
    ...
    flashDuration: 'How long lightning flashes illuminate the scene. Longer values leave more room for return strokes.',
    returnStrokeProbability: 'Chance that a strike produces 1–2 extra return strokes after the first flash.',
    maxReturnStrokes: 'Maximum flash pulses per strike (1–3). Caps how many return strokes can fire.',
    ...
    precipGlowSoftness: 'How gradual the glow edges are (higher = softer feather).',
  };
  ...
  const FOLDER_HELP = {
    ...
    Camera: 'Camera mode, pan speed, horizontal wrapping, and smooth camera motion.',
    ...
    Overlays: 'All on-screen overlay toggles — soundings, risk bars, synoptic markers, storm tracks, weather labels, stations, radars, meteogram, fog/haze, vector field, and help.',
  ...
  const SKY_HELP = {
    ...
    'Sun intensity': 'Scales incoming solar radiation. 1.0 matches Earth (1361 W/m² TOA).',
  ...
  const SOUNDING_CONTROL_HELP = {
    ...
    'togHeights': { title: 'Heights', body: 'Label heights above sea level on the left of the skew-T (not above ground level).' },
